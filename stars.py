...
class Star(Vector):
    ...
    def reverse_abduct(self):
        if self.game.pop > 0:
            ...
            self.card.pop += 1
            self.card.redraw()
            person = Person(self, tracked=True)
            person.reverse_abducted = self.game.time
...
class Person(Vector):
    ...
    def draw(self, dis):
        ...
        if 0 <= x < dis.res.x and 0 <= y < dis.res.y:
            ...
            elif self.reverse_abducted:
                p = 1 - (self.game.time - self.reverse_abducted)/self.abduct_anim_dur
                pg.draw.circle(dis.srf, (255, 255, 255), (x, y), int(p*10))
            else:
                if self.hover:
